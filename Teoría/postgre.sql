...
-- En PostgreSQL se usan comillas dobles para los alias con espacios
SELECT name, init_date AS "Fecha de inicio en programación" FROM users WHERE name = 'Brais';

-- Consulta igual que la anterior.
SELECT name, init_date AS "Fecha de inicio en programación" FROM users WHERE name = 'Brais';
...
-- En PostgreSQL datetime se suele usar como TIMESTAMP
...
-- En PostgreSQL CURRENT_TIMESTAMP no lleva paréntesis obligatoriamente, pero funciona.
...
-- En PostgreSQL se usa SERIAL para enteros autoincrementales
...
-- En PostgreSQL primero definimos la función del trigger y luego el trigger en sí.

CREATE OR REPLACE FUNCTION tg_email() RETURNS TRIGGER AS $$
BEGIN
...
-- En PostgreSQL, para devolver resultados de una consulta se suelen usar funciones que retornan tablas.

CREATE OR REPLACE FUNCTION p_all_users() RETURNS TABLE (
    user_id int,
    name varchar,
    surname varchar,
    age int,
...
-- Invoca a la función llamada "p_all_users"
...
CREATE OR REPLACE FUNCTION p_age_users(age_param int) RETURNS TABLE (
    user_id int,
    name varchar,
    surname varchar,
    age int,
...
BEGIN
	RETURN QUERY SELECT * FROM users WHERE users.age = age_param;
END;
$$ LANGUAGE plpgsql;

-- Invoca a la función llamada "p_age_users" con un parámetro de valor 30
SELECT * FROM p_age_users(30);

-- Elimina la función llamada "p_age_users"
DROP FUNCTION p_age_users;
